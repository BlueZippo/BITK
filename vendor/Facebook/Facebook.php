...
class Facebook
{
    ...
    {
        $config = array_merge([
            'app_id' => Config::get('FACEBOOK_ID'), //getenv(static::APP_ID_ENV_NAME),
            'app_secret' => Config::get('FACEBOOK_SECRET'),
        ...
        if (!$config['app_id']) {
            throw new FacebookSDKException('Required "app_id" key not supplied in config and could not find fallback environment variable "' . Config::get('FACEBOOK_ID') . '"');
        }
        if (!$config['app_secret']) {
            throw new FacebookSDKException('Required "app_secret" key not supplied in config and could not find fallback environment variable "' . Config::get('FACEBOOK_SECRET') . '"');
        }
